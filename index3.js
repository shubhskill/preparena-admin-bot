...
async function showTestSubjects(chatId, testId) {
  ...
  if (subjectsError) {
    console.error("showTestSubjects subjects error:", subjectsError);
    await send(
      chatId,
      `❌ Could not load subjects.\n\n${escapeHtml(subjectsError.message || "Unknown database error")}`
    );
    return;
  }
  ...
  if (selectedError) {
    console.error("showTestSubjects selected subjects error:", selectedError);
    await send(
      chatId,
      `❌ Could not load selected subjects.\n\n${escapeHtml(selectedError.message || "Unknown database error")}`
    );
    return;
  }
