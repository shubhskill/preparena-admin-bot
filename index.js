...
async function processCreateTest(msg) {
  ...
  if (!session || session.state !== "create_title") {
    return false;
  }
  ...
  if (!title) {
    await send(chatId, "❌ Test name cannot be empty.");
    return true;
  }
  ...
  const draft = {
    ...
    // Default values keep the existing schema valid. Duration can be edited
    // from Edit Test before publishing.
...
async function finalizePublishTest(chatId, testId, date, time) {
  ...
  if (!test.total_questions) {
    await send(chatId, "❌ Add at least one question before publishing.");
    return;
  }

  const { data: selectedSubjects, error: subjectError } = await supabase
    .from("test_subjects")
    .select("subject_id")
    .eq("test_id", testId);
  ...
  if (subjectError) {
    ...
    await send(chatId, "❌ Could not verify test subject.");
    return;
  }

  if (!selectedSubjects?.length) {
    await send(chatId, "❌ Select at least one subject before publishing.", {
      reply_markup: {
        inline_keyboard: [[
          { text: "📚 Select Subject(s)", callback_data: `${PREFIX}tsub:${testId}` }
        ]]
      }
    });
    return;
  ...
  if (Number.isNaN(startsAt.getTime())) {
    await send(chatId, "❌ Invalid date/time.");
    return;
  }

  if (startsAt.getTime() <= Date.now()) {
    await send(chatId, "❌ Test date and time must be in the future.");
    return;
  }

  const endsAt = new Date(
    startsAt.getTime() + Number(test.duration_minutes || 180) * 60000
  );
  ...
  await send(
    chatId,
    `🚀 <b>Test Published Successfully</b>\n\n<b>${escapeHtml(test.title)}</b>\n\n📅 Date: ${date}\n⏰ Time: ${time}\n❓ Questions: ${test.total_questions}\n💯 Total Marks: ${test.total_marks}\n\nAt the scheduled time, all active admins and the owner will receive a <b>Start Test</b> button. The test will begin only after an admin or owner presses it.`,
    {
      reply_markup: {
        inline_keyboard: [
...
async function processPublishInput(msg) {
  ...
  if (!session || !["publish_date", "publish_time"].includes(session.state)) {
    return false;
  }
  ...
  if (session.state === "publish_date") {
    ...
    await send(chatId, "⏰ Enter test time (HH:MM):");
    return true;
  }
  ...
  if (session.state === "publish_time") {
    ...
    const testId = session.publishTestId;
    const date = session.publishDate;

    await finalizePublishTest(chatId, testId, date, text);
    return true;
  }
...
async function publishTest(chatId, testId) {
  ...
  setSession(chatId, {
    state: "publish_date",
    publishTestId: testId,
    publishDate: null
  });

  await send(
    chatId,
    `🚀 <b>Publish Test</b>\n\nTest: <b>${escapeHtml(refreshed.title)}</b>\nQuestions: ${refreshed.total_questions}\nTotal Marks: ${refreshed.total_marks}\n\n📅 Now enter the test date (YYYY-MM-DD):`
  );
}
...
async function checkScheduledTests() {
  try {
    ...
    console.error("Scheduled test checker error:", error);
  }
}
...
async function startTestNow(chatId, testId) {
  ...
  const { data: startedRow, error } = await supabase
    ...
    .eq("id", testId)
    .eq("status", "published")
    .in("lobby_state", ["waiting", "ready"])
    .select("id, lobby_state")
    .maybeSingle();
...
bot.on("callback_query", async query => {
  ...
  if (!isPrivateChat(chat)) {
    await bot.answerCallbackQuery(query.id, { text: "Admin Bot sirf private chat me available hai.", show_alert: true }).catch(() => {});
    return;
  }
